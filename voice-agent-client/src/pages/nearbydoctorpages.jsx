...
export default function NearbyDoctorsMap({ user, onBookDoctor }) {
  ...
  const locateUser = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        if (mapObjRef.current && window.L) {
          ...
          userMarkerRef.current = L.marker([lat, lng], {
            ...
            .bindPopup(
              '<span style="font-size:12px;font-weight:600;color:#111">📍 Your Location</span>',
            )
      ...
      (err) => {
        setLocating(false);
        setError(
          `Could not get your location: ${err.message}. Please allow location access and try again.`,
        );
      },
  ...
  const fetchNearby = useCallback(
    async (lat, lng, r = radius) => {
      ...
      try {
        ...
        if (!data.success)
          throw new Error(data.error || "Failed to load nearby doctors");
  ...
  const plotDoctors = useCallback((docs, userLat, userLng) => {
    ...
    if (docs.length > 0 && userLat && userLng) {
      const allCoords = docs
        .filter(
          (d) => d.location?.coordinates && !(d.location.coordinates[0] === 0),
        )
  ...
  const focusDoctor = (doc) => {
    ...
    const [dLng, dLat] = coords;
    if (dLat && dLng)
      mapObjRef.current.setView([dLat, dLng], 16, { animate: true });
  };
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <button
              ...
              style={{
                ...
                alignItems: "center",
                gap: 6,
                background: showFilters ? `${T.accent}15` : T.card,
              ...
              {specs.map((s) => (
                <button
                  ...
                  style={{
                    ...
                    fontWeight: 600,
                    cursor: "pointer",
                    border: `1.5px solid ${specFilter === s ? T.accent : T.border}`,
                    background:
                      specFilter === s ? `${T.accent}15` : "transparent",
                    color: specFilter === s ? T.accent : T.muted,
                    transition: "all .2s",
                  }}
                >
                  {s}
          ...
          >
            ...
            {!userLoc && mapReady && !loading && (
              ...
              >
                ...
                >
                  Click "Locate Me" to find dental specialists near your current
                  location.
                </p>
                <button
                  onClick={locateUser}
                ...
                >
                  ...
                  >
                    {doc.experience > 0 && (
                      ...
                      >
                        <FaClock style={{ fontSize: 9, color: T.gold }} />
                        {doc.experience} yrs
                      </span>
                    )}
                    {doc.consultationFee && (
          ...
          {selected && (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 200 }}>
                ...
                <p style={{ color: T.accent, fontSize: 12, marginBottom: 6 }}>
                  {selected.specialization} · {selected.qualification} ·{" "}
              ...
              {selected.distanceKm && (
                ...
                >
                  ...
                  <span
                    style={{ color: T.emerald, fontWeight: 700, fontSize: 14 }}
                  >
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button
                  onClick={() => onBookDoctor && onBookDoctor(selected)}
                  style={{
                    background: `linear-gradient(135deg,${T.accent},#0066AA)`,
                    color: T.bg,
                    border: "none",
