...
router.post("/register", async (req, res) => {
  try {
    ...
    console.log(
      `✅ New doctor registered: ${doctor.name} (${doctor.email}) — coords: [${resolvedLocation?.coordinates || "0,0"}]`,
    );
      ...
      for (const doc of zeroDocs) {
        ...
        await Doctor.findByIdAndUpdate(doc._id, {
          $set: {
            ...
            "location.address":
              doc.location?.address || geo.fullAddress || address,
        ...
        console.log(
          `  ✅ ${doc.name} → [${geo.coordinates}] (${geo.placeName})`,
        );
