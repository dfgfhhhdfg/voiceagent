...
async function fixDoctorGeo() {
  try {
    ...
    if (badDocs.length) {
      console.log(`🔧 Fixing ${badDocs.length} doctor(s) with corrupt location data…`);
      for (const doc of badDocs) {
        ...
        console.log(`  ✅ Fixed: ${doc.name} → coordinates ${isValidCoords ? coords : [0,0]}`);
      }
    }
    ...
    for (const idx of indexes) {
      ...
      const isCorrect = idx.key.location === '2dsphere' && Object.keys(idx.key).length === 1;
      if (is2d && !isCorrect) {
