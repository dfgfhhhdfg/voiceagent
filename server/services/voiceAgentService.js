...
function normalizeSpokenEmail(transcript) {
  ...
  while (i < tokens.length) {
    ...
    const twoWord =
      i + 1 < tokens.length ? `${token} ${tokens[i + 1].toLowerCase()}` : null;
  ...
  const hasDot =
    result.includes(".") && result.indexOf(".") > result.indexOf("@");
...
class VoiceAgentService {
  constructor() {
    ...
    if (!key)
      throw new Error(
        "GROQ_API_KEY missing from .env  ->  https://console.groq.com/keys",
      );
    ...
    this.tools = [
      {
        ...
        function: {
          name: "get_doctors",
          description:
            "Get list of all available doctors with their specialization, address and clinic location.",
          parameters: {
            type: "object",
            properties: {
              specialization: {
                type: "string",
                description:
                  "Optional filter by specialization — omit entirely if not specified",
              },
            },
            required: [],
          },
      ...
      },
      {
        ...
        function: {
          ...
          description:
            "Find doctors near a given location (lat/lng) within a radius. Use when patient asks for doctors near them or near a city.",
          parameters: {
            type: "object",
            properties: {
              lat: {
                type: "number",
                description: "Latitude of patient or place",
              },
              lng: {
                type: "number",
                description: "Longitude of patient or place",
              },
              radiusKm: {
                type: "number",
              ...
              specialization: {
                type: "string",
                description:
                  "Optional specialization filter — omit if not specified",
              },
              locationName: {
                type: "string",
                description: 'Human-readable place name e.g. "Mysuru"',
              },
            },
            required: ["lat", "lng"],
          },
        ...
        function: {
          ...
          parameters: {
            ...
            properties: {
              ...
              doctorId: {
                type: "string",
                description: "Optional MongoDB ObjectId of doctor",
              },
            },
            required: ["date"],
          },
        ...
        function: {
          ...
          parameters: {
            ...
            properties: {
              ...
              doctorId: {
                type: "string",
                description: "MongoDB ObjectId of doctor (preferred)",
              },
              doctorName: {
                type: "string",
                description: "Doctor name fallback if no doctorId",
              },
              notes: { type: "string" },
            },
            required: [
        ...
        function: {
          ...
          parameters: {
            ...
            properties: {
              appointmentId: {
                type: "string",
                description: "MongoDB ObjectId — only if you actually have it",
              },
              patientEmail: {
                type: "string",
                description: "Patient email to look up their appointment",
              },
              date: {
                type: "string",
                description: "YYYY-MM-DD — helps narrow down which appointment",
              },
              reason: { type: "string" },
            },
            required: [],
        ...
        function: {
          ...
          parameters: {
            ...
            properties: {
              infoType: {
                type: "string",
                enum: ["address", "phone", "hours", "emergency", "all"],
              },
            },
            required: ["infoType"],
          },
...
- Always wait for tool results before responding.`;
    ...
    if (patientContext?.name || patientContext?.email) {
      ...
      if (patientContext.lat && patientContext.lng) {
        known.push(
          `Location: lat ${patientContext.lat}, lng ${patientContext.lng}`,
        );
      }
...
2. Call cancel_appointment with those details`;
    ...
    for (const model of available) {
      try {
        ...
        while (assistantMsg.tool_calls?.length) {
          console.log(
            `🔧 Tools: ${assistantMsg.tool_calls.map((t) => t.function.name).join(", ")}`,
          );
        ...
        if (!text) {
          msgList.push({
            role: "user",
            content:
              "Please give a brief spoken response to the patient based on the tool result above.",
          });
  ...
  async processUserInput(
    ...
    try {
      ...
      if (cleaned.length < 3)
        return { text: null, conversationId, suppressed: true };
  ...
  async executeTool(name, args) {
    try {
      switch (name) {
        case "get_doctors":
          return await this.getDoctors(args.specialization);
        case "find_nearby_doctors":
          return await this.findNearbyDoctors(args);
        case "check_availability":
          return await this.checkAvailability(args.date, args.doctorId);
        case "book_appointment":
          return await this.bookAppointment(args);
        case "cancel_appointment":
          return await this.cancelAppointment(args);
        case "get_clinic_info":
          return this.getClinicInfo(args.infoType);
        default:
  ...
  async getDoctors(specialization = undefined) {
    ...
    const doctors = await Doctor.find(query)
      ...
      .sort({ rating: -1 });
    if (!doctors.length)
      return { success: true, doctors: [], message: "No doctors found." };
  ...
  async findNearbyDoctors({
    ...
    const list = doctors.map((d) => {
      ...
      return {
        ...
        qualification: d.qualification,
        clinicAddress:
          d.location?.address || d.address || "Address not provided",
        placeName: d.location?.placeName || "",
        phone: d.phone,
        distanceKm: distKm,
      };
  ...
  async bookAppointment(data) {
    ...
    if (!isValidEmail(cleanedEmail)) {
      return {
        success: false,
        message: `The email "${rawEmail}" doesn't look valid. Please confirm.`,
      };
    }
    ...
    let doctor = null;
    if (data.doctorId)
      doctor = await Doctor.findById(data.doctorId).catch(() => null);
    ...
    if (doctor?.email) {
      try {
        ...
        console.warn("⚠️ Doctor email failed:", e.message);
      }
    }
  ...
  async cancelAppointment({ appointmentId, patientEmail, date, reason = "" }) {
    ...
    if (!appt) {
      return {
        success: false,
        message:
          "No scheduled appointment found. Please check the details and try again.",
      };
    }
    ...
    try {
      await emailService.sendCancellationEmail(appt, {
  ...
  getClinicInfo(infoType) {
    const info = {
      address:
        ...
        "123 Dental Ave, Suite 100, New York, NY 10001",
